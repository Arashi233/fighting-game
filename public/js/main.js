...
$(function() {
	...
	var id = $('#account').val(localStorage.account);
	$('#content').hide();
	$('#login').show();
	$('#tips').hide();
	...
	socket.on('start', function(json) {
		...
		stepTime = 0;
		showTips('game start');
		socket.emit('param', { id: $('#account').val() });
	...
	socket.on('timeSync', function(json) {
		...
		var delay = Date.now() - client;
		if (avgDelayCount == avgDelayMax) {
			$('#lag').text('delay: ' + Math.ceil(avgDelay / avgDelayMax) + ' ms');
			avgDelay = 0;
			avgDelayCount = 0;
		}
	...
	function sendCommand() {
		if (isFastRunning) {
			console.log('Delaying send while catching up');
			return;
		}
	...
	$('#start_btn').click(function() {
		...
		if (isConnected == false) {
			showTips('failed to connect to server');
		} else if (currentAccount == '') {
			showTips('account cannot be empty');
		} else {
